...
> **문서 버전**: 1.1
> **작성일**: 2026-05-06 / 갱신: 2026-05-08
> **기반 문서**: PRD.md v0.2, ARCHITECTURE.md v0.2, AGENTS.md v0.1
> **상태**: M0~M5 완료 (post-M5 버그 수정 포함)
...
M5 완료 후 실제 EXE 실행 테스트에서 발견된 결함:

| # | 버그 | 원인 | 수정 커밋 |
|---|------|------|---------|
| B-1 | 스캔 후 트리뷰 빈칸 | `LazyTreeModel.add_node()`가 DFS 후위순회 첫 노드(리프)를 루트로 설정 → `rowCount()=0` | `a967304` |
| B-2 | EXE 테마 없음 | `theme_manager._THEMES_DIR = Path(__file__).parent`가 onefile EXE 내부에서 유효하지 않음 | `a967304` |
| B-3 | EXE 자체가 실행되지 않음 | PyInstaller 진입점이 `app.py`로 설정되어 있어 `run()`이 호출되지 않고 임포트 4초 후 exit 0 | `956fe44` |
| B-4 | qtawesome 아이콘 오류 | 아이콘 접두어 `fa.` → `fa5s.` 변경 필요 (FA4→FA5 API 변경) | `a4b5ea1` |
...
| 1.1 | 2026-05-08 | 상태 갱신(완료), post-M5 버그 수정 이력 §13 추가 |
